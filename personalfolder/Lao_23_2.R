...
for(rep in 1:100)
{
  ne <- round(runif(1,100,20000));  #1 to 20000 chromosoms
  d <- model.single.pop(ne, 5)$haplotype_matrix;
  n <- nrow(d);
  nsnps <- ncol(d);
  distance <- sum(as.matrix(dist(d)))/(n*(n-1));  #dist = euclidean distance
  heterozygosity.stat[rep] <- distance; # average distance between every pair of haplotype
  ne.parameter[rep] <- ne;
}
